...
# ── AWS Free Tier Memory & Bandwidth Limits ───────────────────────
CHUNK_SIZE             = 512 * 1024                         # 512 KB chunks (Lower RAM usage)
MAX_CONCURRENT_STREAMS = int(os.getenv("MAX_CONCURRENT_STREAMS", "3")) # Strict limit for 1GB RAM
MAX_CONCURRENT_THUMBS  = int(os.getenv("MAX_CONCURRENT_THUMBS", "4"))
MAX_THUMB_CACHE        = int(os.getenv("MAX_THUMB_CACHE", "50"))
MAX_DOC_CACHE          = int(os.getenv("MAX_DOC_CACHE", "50"))
READAHEAD_CHUNKS       = int(os.getenv("READAHEAD_CHUNKS", "1")) # Stop downloading if user pauses
...
async def lifespan(app: FastAPI):
    ...
    bot_app = Client(
        ...
        bot_token=BOT_TOKEN,
        in_memory=False,  # CRITICAL FOR AWS: Must save to disk to avoid FloodWait
    )
    try:
        await bot_app.start()
    ...
    if SESSION_STRING:
        stream_client = Client(
            ...
            in_memory=False,  # CRITICAL FOR AWS
        )
        try:
            await stream_client.start()
            _clients_started["stream"] = True
            log.info("✅ User session started — streaming via user.")
        except FloodWait as e:
...
async def build_stream_response(
    ...
    # AWS Bandwidth Savings: Aggressive Cache-Control
    headers = {
        "Accept-Ranges": "bytes",
...
# ===================================================================
# THUMBNAILS (AWS Tuned with Fail-Fast)
# ===================================================================
...
async def get_thumb(vid: str):
    ...
    # FAIL FAST: If we know there's no thumbnail, don't hit Telegram.
    if not doc.get("thumb_file_id"):
    ...
    try:
        ...
        # 3. Fallback: Fetch message fresh
        try:
